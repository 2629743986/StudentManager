...
<form action="servlet/ExportWordServlet.servlet" method="post">
    ...
    <table width="100%" cellpadding="0" cellspacing="0">
    	<tr>
            <td align="left" colspan="5" style="color: red;">登录信息：
            <c:if test="${student_admin_message.studentstate==1 }">帐号启用</c:if>
            <c:if test="${student_admin_message.studentstate==2 }">帐号禁用</c:if>
            <a href="AdminStudentState?studentId=${student_admin_message.studentId }" style="color: blue;">启用学生帐号</a>
            </td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td colspan="3">
            	${student_admin_message.political.politicallandscape }&nbsp;
            	 <a href="updateSutdentPoliticall?studentId=${student_admin_message.studentId }" style="color: blue;" onclick="return confirm('确定审批通过吗?')">点击进行审批</a><br/>
            	 
            	 <c:choose>  
				   <c:when test="${student_admin_message.politicallandscapestate==4}">  
				   <p style="color: red;">(没审批)</p>  
				   </c:when>  
				     
				   <c:when test="${student_admin_message.politicallandscapestate==1}">  
				   <p style="color: red;">(审批通过)</p>  
				   </c:when>  
				    <c:when test="${student_admin_message.politicallandscapestate==2}">  
				   <p style="color: red;">(辅导员审批通过)</p>  
				   </c:when> 
				    <c:when test="${student_admin_message.politicallandscapestate==3}">  
