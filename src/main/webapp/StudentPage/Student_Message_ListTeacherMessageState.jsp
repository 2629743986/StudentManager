 	...
 	<script type="text/javascript">
	    function validate(){
	        ...
	        if(page > <s:property value="#request.pageBean.totalPage"/>){
	            ...
	            window.document.location.href = "TeacherNoReply?studentId=${ student_message.studentId}";
	            return false;
	        }
	        return true;
	    }
...
<div class="banner">
    ...
    <div class="morebt">
        <ul>
           	<li>
           		<form action="showPageTeacherMessage" method="post" name="from8" id="from8">
				<input type="hidden" name="studentId" value="${student_message.studentId}"/>
				<a href="javascript:document:from8.submit();" class="tab2">辅导员咨询列表</a>
				</form>
           	 </li>
           <%-- 	<li>
           		<form action="showPageTeacherMessage" method="post" id="from9" name="from9">
                 <input type="hidden" name="studentId" value="${ student_message.studentId}"/>
                 <a href="javascript:document:from9.submit();" class="tab2">助理辅导员咨询列表</a>
            </form>
			</li>
           	<li>
           		<form action="StudentAdminmessagePages" method="post" id="from6" name="from6">
                    <input type="hidden" name="studentId" value="${ student_message.studentId}"/>
                      <a href="javascript:document:from6.submit();" class="tab2">辅导员已处理列表</a>
                </form>
           
           	 </li> --%>
           	 <li>
           		<form action="TeacherNoReply" method="post" id="from7" name="from7">
                    <input type="hidden" name="studentId" value="${ student_message.studentId}"/>
                      <a href="javascript:document:from7.submit();" class="tab2">辅导员已处理列表</a>
                </form>
    ...
    <div class="cztable">
            ...
            <tbody>
              ...
              <s:iterator value="#request.pageBean.list" id="Teacherreply">
			        <tr>
			            ...
			            <th scope="col"> 
			            	<a href="StudentReply?teacherMessageId=<s:property value="#Teacherreply.teacherReplyId"/>&teacherReplyId=<s:property value="#Teacherreply.teachermessage.teacherMessageId"/>" style="color: red" >详情</a>
                   		</th>       
			        </tr>
      			</s:iterator>
            </tbody>
        ...
        <s:else>
            <a href="TeacherNoReply?studentId=${ student_message.studentId}">首页</a>
            &nbsp;&nbsp;&nbsp;
            <a href="TeacherNoReply?page=<s:property value="#request.pageBean.currentPage - 1"/>&studentId=${student_message.studentId }">上一页</a>
        </s:else>
        
        <s:if test="#request.pageBean.currentPage != #request.pageBean.totalPage">
            <a href="TeacherNoReply?page=<s:property value="#request.pageBean.currentPage + 1"/>&studentId=${student_message.studentId }">下一页</a>
            &nbsp;&nbsp;&nbsp;
            <a href="TeacherNoReplye?page=<s:property value="#request.pageBean.totalPage"/>&studentId=${student_message.studentId }">尾页</a>
        </s:if>
