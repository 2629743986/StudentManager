...
function httpRequest(reqType,url,async,resFun,parameter){	
	...
	function checkName(){
		var name=document.getElementById("professionName").value;
			//请求参数
		    request=httpRequest("post","servlet/CheckProfessionName.servlet?professionName="+name,true,callbackFunc);//调用Ajax提交请求方法  
		    if(document.getElementById("professionName").value == ""){
				alert("专业名称不能为空");
				return false;
			}
		    if(rs == true){
